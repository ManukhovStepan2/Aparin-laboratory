...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   ofstream os("pion_yield.dat");       //deepti
   ...
   TH2F *hvxvy = new TH2F("hvxvy","Vx vs Vy " ,150, -3., 3.,150, -3., 3.);
   ...
   //---------------------//
   Float_t mp = Mpion; //change mass of particle here
   //----- Geant ID ------//
   //piplus = 8,  piminus = 9
   //kplus  = 11, kminus = 12
   //proton = 14, pbar = 15
   //--------------------//
   Int_t Gid = 8; //change Gid here
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) 
	...
	tcountmctrk = 0;
       ...
       for(Int_t mc = 0; mc < mMcTracks_; mc++)
	 {
	   ...
	   // cout<<"mc_GID = "<<mMcTracks_mGeantId[mc]<<"\t mc_Parent_GID = "<<mMcTracks_mParentGeantId[mc]<<endl;
	   if(mMcTracks_mGeantId[mc] != Gid  || mMcTracks_mParentGeantId[mc]!=0)
             {
       ...
       for(Int_t i = 0; i < mMatchedPairs_ ; i++)
	 {
	   ...
	   */
	   ...
	   //------ Check Geant Id ----------//
	   //cout<<"match_GID = "<<mMatchedPairs_mGeantId[i]<<"\t match_parent_GID = "<<mMatchedPairs_mParentGeantId[mc]<<endl;
       ...
       os<<gevt+1<<"\t"<<countmatch<<"\t"<<mNUncorrectedPrimaries<<endl;     //deepti
