...
// Sat Oct  4 15:52:49 2008 by ROOT version 5.12/00h-rc2
// from TTree mTrackEvent/trackTree
// found on file: test.rectree.root
//////////////////////////////////////////////////////////

#ifndef mTrackEvent_h
#define mTrackEvent_h
...
public :
   ...
   Float_t         MagField;
   Int_t           mNMcTrk;
   Int_t           mMcId[5000];   //[mNMcTrk]
   Int_t           mGeantId[5000];   //[mNMcTrk]
   Int_t           mParentGeantId[5000];   //[mNMcTrk]
   Int_t           mParentId[5000];   //[mNMcTrk]
   Float_t         mMcPt[5000];   //[mNMcTrk]
   Float_t         mMcEta[5000];   //[mNMcTrk]
   Float_t         mMcPhi[5000];   //[mNMcTrk]
   Float_t         mMcParentPt[5000];   //[mNMcTrk]
   Float_t         mMcParentEta[5000];   //[mNMcTrk]
   Float_t         mMcParentPhi[5000];   //[mNMcTrk]
   Int_t           mMcParentO[5000];   //[mNMcTrk]
   Float_t         mMcMass[5000];   //[mNMcTrk]
   Float_t         mMcStartX[5000];   //[mNMcTrk]
   Float_t         mMcStartY[5000];   //[mNMcTrk]
   Float_t         mMcStartZ[5000];   //[mNMcTrk]
   Int_t           mMcNhits[5000];   //[mNMcTrk]
   Int_t           mNRcTrk;
   Int_t           mRcId[5000];   //[mNRcTrk]
   Int_t           mRcAssoId[5000];   //[mNRcTrk]
   Int_t           mRcAssoOffset[5000];   //[mNRcTrk]
   Float_t         mRcPt[5000];   //[mNRcTrk]
   Float_t         mRcEta[5000];   //[mNRcTrk]
   Float_t         mRcPhi[5000];   //[mNRcTrk]
   Int_t           mRcNhits[5000];   //[mNRcTrk]
   Int_t           TPCrightTrack[5000];   //[mNRcTrk]
   Int_t           sharedTpcHits[5000];   //[mNRcTrk]
   Float_t         percentSharedTpcHits[5000];   //[mNRcTrk]
   Float_t         mHelixCurv[5000];   //[mNRcTrk]
   Float_t         mHelixDip[5000];   //[mNRcTrk]
   Float_t         mHelixPhase[5000];   //[mNRcTrk]
   Float_t         mHelixX[5000];   //[mNRcTrk]
   Float_t         mHelixY[5000];   //[mNRcTrk]
   Float_t         mHelixZ[5000];   //[mNRcTrk]
   Float_t         mArc[5000];   //[mNRcTrk]
   Float_t         mSDCA[5000];   //[mNRcTrk]
   Float_t         mSDCA1[5000];   //[mNRcTrk]
   Float_t         mSDCA2[5000];   //[mNRcTrk]
...
#ifdef mTrackEvent_cxx
mTrackEvent::mTrackEvent(TTree *tree)
{
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      ...
      tree = (TTree*)gDirectory->Get("mTrackEvent");

   }
   Init(tree);
}

mTrackEvent::~mTrackEvent()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t mTrackEvent::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t mTrackEvent::LoadTree(Long64_t entry)
{
...
void mTrackEvent::Init(TTree *tree)
{
...
void mTrackEvent::Show(Long64_t entry)
{
...
Int_t mTrackEvent::Cut(Long64_t entry)
{
