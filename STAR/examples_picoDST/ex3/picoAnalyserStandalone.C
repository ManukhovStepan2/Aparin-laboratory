 ...
 *
 * picoAnalyser.C is an example of reading STAR picoDst format in a standalone mode
 * on your laptop of local computer farm.
 * Prerequisites:
 * - StPicoEvent directory
 * - CERN ROOT package
 * - g++ >= 4.8
 * - Makefile
 *
 ...
 * make
 *
 * Then the executable file picoAnalyserStandalone will be created. The current version of the program
 * expects 3 arguments: ./picoAnalyserStandalone inputFile outputFile
...
int main(int argc, char* argv[]) {
  ...
  case 3:
    ...
    break;
  default:
    std::cout << "Usage: picoAnalyserStandalone inputFileName outputFileName.root" << std::endl;
    return -1;
  }
